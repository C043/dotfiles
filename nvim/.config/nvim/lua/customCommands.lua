...
vim.opt.viewoptions = { "folds", "cursor", "curdir" } -- what to restore
...
vim.cmd([[
function! MarkdownH2FoldExpr() abort
  let l = getline(v:lnum)

  " H1 ends any higher-level folding (level 0)
  if l =~# '^#\s'
    return 0
  endif

  " Start a new fold at H2 (level 1)
  if l =~# '^##\s'
    return '>1'
  endif

  " H3/H4 nest under the current H2
  if l =~# '^###\s'
    return 2
  endif
  if l =~# '^####\s'
  ...
  " All other lines inherit previous line's fold level
  return '='
endfunction
]])
...
vim.api.nvim_create_autocmd("FileType", {
	...
	callback = function()
		...
		vim.opt_local.foldexpr = "MarkdownH2FoldExpr()"
		vim.opt_local.foldenable = true
		vim.opt_local.foldlevel = 99 -- start unfolded; set to 0/1 to start closed
		vim.opt_local.foldminlines = 0 -- allow tiny folds
		vim.opt_local.foldignore = "" -- don't ignore blank lines
	end,
})
